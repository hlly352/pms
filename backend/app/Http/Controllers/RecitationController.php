...
class RecitationController extends Controller
{
    ...
    {
        // 1. 获取文档数据
        $doc = Recitation::findOrFail($id);

        // 如果是文件夹，不让打印
        if ($doc->type === 'folder') {
            return response()->json(['message' => '文件夹无法生成 PDF'], 400);
        }

        // 2. 生成 PDF (你需要提前去 resources/views/reports/ 下建一个 recitation_doc.blade.php 模板文件)
        $pdf = PDF::loadView('reports.recitation_doc', [
            'doc' => $doc,
        ], [], [
            'format'           => 'A4',
            'margin_left'      => 15,
            ...
            'autoLangToFont'   => true, // 非常重要：Markdown 里通常有中文，必须开启以防乱码
        ]);

        $filename = "document_{$doc->id}.pdf";
        ...
        // 默认直接 stream 出流数据，Vue 前端会以 Blob 形式接收它
