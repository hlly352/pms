...
class ProjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Project::with([
            ...
            'timeAccount', 
            'account', 
            'transactions.account', // 🌟 新增：加载项目的真实资金流水及其扣款账户（供资金消耗 Tab 使用）
            'stages.segments', 
            'stages.steps.task' => function($q) {
                $q->with('details') // 🌟 新增：深度加载每日打卡明细（供时间消耗树形表使用）
                  ->withCount([
    ...
    {
        // 1. 验证数据 (🌟 增加了 account_id 验证)
        $validated = $request->validate([
            'goal_id' => 'required|exists:goals,id',
            'name' => 'required|string',
            'planned_budget' => 'nullable|numeric|min:0',
            'time_account_id' => 'nullable|integer', 
            'account_id' => 'nullable|integer', // 👈 新增验证：关联的财务账户
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($request) {
            // A. 创建项目主表 (🌟 增加了 account_id)
            $project = Project::create([
                'goal_id' => $request->goal_id,
                'name' => $request->name,
                'planned_budget' => $request->planned_budget,
                'time_account_id' => $request->time_account_id, 
                'account_id' => $request->account_id, // 👈 记录绑定的财务账户
                'expected_result' => $request->expected_result,
                'goal_weight' => $request->goal_weight,
                'start_date' => $request->start_date,
                'end_date' => $request->end_date,
